...
export default function ChatPage() {
  ...
  // These callbacks are passed to useMobileNavigation and must be stable
  ...
  // Computed online status - only show if:
  // 1. We've received a reply from the recipient (basic privacy)
  // 2. OUR online status is enabled (reciprocal privacy - WhatsApp/Signal model)
  ...
  // Reciprocal privacy: You can only see others' online/last seen if you share yours
  // Online Status setting controls both indicators (simplified from separate lastSeen setting)
  ...
  // Mark conversation as read locally when selected (but don't send receipts yet)
  // Read receipts are sent via onMessageVisible when messages are actually viewed
  useEffect(() => {
  ...
  // Handle when a message becomes visible in the viewport (for read receipts)
  const handleMessageVisible = useCallback((message: Message) => {
    // Only send read receipts for ACCEPTED conversations (privacy protection)
  ...
  // Handle conversation selection - click again to deselect (close chat)
  const handleSelectConversation = useCallback((id: string) => {
    if (isMobile) {
      navigateToChat(id);
    } else {
      // Toggle: if clicking on already-selected conversation, deselect it
  ...
  const handleSelectUser = useCallback((foundUser: {
    ...
    // Close the dialog FIRST before any state changes
    ...
    // Add conversation and select it using mobile-aware navigation
  ...
  // Handle file download - opens lightbox for images, downloads other files
  const handleFileDownload = useCallback(async (metadata: import('@/core/types').FileMetadata) => {
    ...
    try {
      ...
      if (metadata.mimeType.startsWith('image/')) {
        ...
        // Don't revoke URL - the lightbox needs it
      } else {
  ...
  // Handle lightbox close - revoke blob URL
    ...
    <div className="h-screen-dynamic flex bg-background overflow-hidden">
      ...
      >
        {activeConversation ? (
          <>
            ...
            {/* Typing Indicator - Premium animated (only for accepted conversations) */}
            {activeConversation &&
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          ...
          // Clear search state when dialog closes (for Cancel/click outside)
          if (!open) {
            setNewChatUsername('');
            setSearchResults([]);
          }
...
/**
 * Empty state when no conversation is selected - Premium Design
 */
function EmptyState({
  onNewChat,
  isMobile,
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 relative">
      ...
      <div className="text-center max-w-sm relative">
        ...
        {/* Encryption badge - below button, centered */}
